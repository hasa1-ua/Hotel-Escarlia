...
<div class="separation">
    <button class="button1" onclick="window.location.href='/Webmaster/menu-reservas/temporadas/crear';">Crear</button>
</div>
<table class="tabla-tipo">
    ...
    @forelse ($temporadas as $temporada)
        <tr onclick="window.location.href='/Webmaster/menu-reservas/temporadas/editar/{{$temporada->id}}';" style="cursor: pointer;">
                <!-- Realizar vista descripcion y añadir una columna mas para verla-->
                <!-- Añadir añadir, editar y borrar-->
                <td>{{ $temporada->id }}</td>
                <td>{{ $temporada->nombre }}</td>
                <td>{{  $temporada->multiplicador }}</td>
                ...
                <td>
                    ...
                    <form action="/Webmaster/menu-reservas/temporadas/{{ $temporada->id }}" method="POST" style="display: inline;">
                        ...
                        <button type="submit" onclick="return confirm('¿Estás seguro de que deseas eliminar esta temporada?');">
...
<div>
    @include('paginate.personalpaginate', ['paginable' => $temporadas])
</div>
</div>
</div>
