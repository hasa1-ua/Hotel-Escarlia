...
<div class="menu">
    <a href="" id="reservas-tab" onclick="showTable('reservas')" class="activo">Reservas</a>
    <a href="" id="cupones-tab" onclick="showTable('cupones')">Cupones</a>
    <a href="" id="temporadas-tab" onclick="showTable('temporadas')">Temporadas</a>
    <a href="" id="regimenes-tab" onclick="showTable('regimenes')">Regimenes</a>
</div>
...
<div class="separation">
    <button class="button1" onclick="window.location.href='/Webmaster/menu-reservas/reservas/crear';">Crear</button>
    <form class="filtro" method="GET" action="{{ url('/Webmaster/menu-reservas/reservas') }}">
    <!-- Filtro para estados -->
    <select name="estado" class="button1">
        <option value="">Seleccionar estado</option>
        <option value="Confirmada"{{ request()->get('estado') == 'Confirmada' ? 'selected' : '' }}>Confirmada</option>
        <option value="Pendiente" {{ request()->get('estado') == 'Pendiente' ? 'selected' : '' }}>Pendiente</option>
...
<table class="tabla-tipo">
    ...
    @forelse ($reservas as $reserva)
        <tr onclick="window.location.href='/Webmaster/menu-reservas/reservas/editar/{{$reserva->id}}';" style="cursor: pointer;">
                ...
                <td>
                    ...
                    <!-- Si tiene habitación pero no sala, mostramos el número de habitación -->
                        {{ $reserva->habitacion->numero }}
                        @else
                    <!-- Si tiene sala pero no habitación, mostramos el nombre de la sala -->
                    {{ $reserva->sala->nombre }}
                        @endif
                </td>
                <td>
                    @if ($reserva->cupon) 
                        <p>{{ $reserva->cupon->descuento }}%</p> <!-- Muestra el descuento de cada cupón -->
                    @else
                        No tiene cupones
                    @endif
                </td>
